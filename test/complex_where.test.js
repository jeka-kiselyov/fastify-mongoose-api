...
test('GET collection complex where', async t => {
	...
	t.equal(response.body.total, 1, 'API returns 1 filtered');
	t.equal(response.body.items.length, 1, 'API returns 1 filtered');
	t.match(response.body.items[0], {name: 'Bob', appleCount: 1, bananaCount: 2}, "Filtered");


	response = await supertest(fastify.server)
		.get('/api/wheretests')
		.query({ where: JSON.stringify({appleCount: {$gt: 10}}) }) //// URL GET parameters
		.expect(200)
		.expect('Content-Type', 'application/json; charset=utf-8')

	t.equal(response.body.total, 1, 'API returns 1 filtered');
	t.equal(response.body.items.length, 1, 'API returns 1 filtered');
	t.match(response.body.items[0], {name: 'Alice', appleCount: 50, bananaCount: 90}, "Filtered");

	response = await supertest(fastify.server)
		.get('/api/wheretests')
		.query({ where: JSON.stringify({$and: [{appleCount: {$gt: 1}}, {bananaCount: {$lt: 5}}]}) }) //// URL GET parameters
		.expect(200)
		.expect('Content-Type', 'application/json; charset=utf-8')

	t.equal(response.body.total, 1, 'API returns 1 filtered');
	t.equal(response.body.items.length, 1, 'API returns 1 filtered');
	t.match(response.body.items[0], {name: 'Rob', appleCount: 2, bananaCount: 3}, "Filtered");
});
